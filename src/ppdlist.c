...
Sppd *open_ppd(char *fname)
{
  ...
  while (fgets(buf, BUFSIZ, fp)) {
	...
	if (!bpt) {
      ...
      if (fcmode == '\0') {
	if (n != TRIANGLE) {
	  fprintf(stderr, "vertex number of Face No.%d is not %d. \n \
                please set vertex number of all faces %d by using \<ppdtri\> !\n",
		  id + 1, TRIANGLE, TRIANGLE);
	  exit(1);
	}
	fc = ofc[id].fc; ++id;
	for (i = 0; i < n; ++i) {
...
int gppd_ok( char *str )
{
  ...
  if (swin->opend == SMDSAVE) {
    if ( swin->morph_ppd != (Sppd *) NULL ) {
      write_ppd_file( str, swin->morph_ppd );
      return SMD_ON;
    } else {
      displayinfo("morph ppd doesn't exist.\n");
      return SMD_OFF;
    }
  } else {
    if ( swin->morph_ppd != (Sppd *) NULL ) {
...
int ppd_ok( char *str, int id )
{
  ...
  if (swin->opend == SMDSAVE) {
    if ( swin->screenatr[id].current_ppd != (Sppd *) NULL ) {
      write_ppd_file( str, swin->screenatr[id].current_ppd );
      strcpy( swin->screenatr[id].current_ppd->filename, str );
      return SMD_ON;
    } else {
      displayinfo("ppd%d doesn't exist.\n", id+1);
      return SMD_OFF;
    }
    if (swin->morph_ppd != (Sppd *) NULL) {
      write_ppd_file(str, swin->morph_ppd);
    ...
    if (swin->screenatr[id].current_ppd != (Sppd *) NULL) {
      free_ppd(swin->screenatr[id].current_ppd);
    }
    if ((ppd = open_ppd(str)) != (Sppd *) NULL) {
      swin->screenatr[id].current_ppd = ppd;
      swin->screenatr[id].view_ppd = ppd;
    }
  }
  return SMD_ON;
}

...
{
  void free_ppdqtree_child(QTree *, Sppd *);
...
void free_ppdqtree_child(QTree *qtree, Sppd *ppd)
{
  int  i;
  void free_ppdqtree_child(QTree *, Sppd *);
  void free_qedge(QTree *);
  void free_qface(QTree *);
  void free_qvertex(QTree *);
...
{
  QVertex *qv, *nqv;
  void  delete_qtree_vertex(QVertex *, QTree *);
...
QTree *init_qtree(Sppd *ppd)
{
  ...
  double xsub, ysub;
  Spvt   *v;
  QTree  *create_qtreechild(QTree *, int, int, 
			    double, double, 
			    double, double);
...
{
  Spvt *vt;
  void decide_qvertex_child(QTree *, Spvt *, Sppd *);
...
void decide_qvertex_child(QTree *qt, Spvt *vt, Sppd *ppd)
{
  ...
  QRange  newqr;
  void    decide_qvertex_child(QTree *, Spvt *, Sppd *);
  QTree   *create_qtreechild_qr(QTree *, int, int, QRange *);
  QVertex *insert_qtree_vertex(Spvt *, QTree *);
  void    subdivide_qrange(QRange *, int, QRange *);
...
{
  Sped  *ed;
  void  decide_qedge_child(QTree *, Sped *, Sppd *);
  
  for (ed = ppd->sped; ed != (Sped *) NULL; ed = ed->nxt) {
/*     display("(ed) %d (vec) (x) %f %f (y) %f %f\n", */
/* 	    ed->no, */
/* 	    ed->sv->uvw.x, */
/* 	    ed->ev->uvw.x, */
/* 	    ed->sv->uvw.y, */
...
void decide_qedge_child(QTree *qt, Sped *ed, Sppd *ppd)
{
  ...
  void   decide_qedge_child(QTree *, Sped *, Sppd *);
  QTree  *create_qtreechild_qr(QTree *, int, int, QRange *);
  QEdge  *insert_qtree_edge(Sped *, QTree *);
  void   subdivide_qrange(QRange *, int, QRange *);
  
  if (qt->level == ppd->maxqtlevel) {
/*     display("(insert1) ed = %d, qt = %d\n", ed->no, qt->level); */
...
QTree *init_hgppdqtree( HGfc *hgfc )
{
  QTree  *head;
  int    level;
  QTree  *create_qtreechild( QTree *, int, int, double, double, double, double );
...
{
  HGed *hged;
  void  decide_hgqedge_child( QTree *, Sped *, HGfc * );
...
{
  void free_hgppdqtree_child( QTree *, HGfc * );
...
{
  HGsf *sf;
  void decide_qface_child(QTree *, HGsf *, HGfc * );
...
void decide_qface_child( QTree *qt, HGsf *sf, HGfc *hgfc )
{
  ...
  QFace  *qf;
  QRange newqr;
  int    hg_triangle_square_intersection( HGsf *, QRange * );
  void   decide_qface_child( QTree *, HGsf *, HGfc * );
  QTree  *create_qtreechild_qr(QTree *, int, int, QRange *);
  QFace  *insert_qtree_face( HGsf *, QTree * );
  void   subdivide_qrange( QRange *, int, QRange * );

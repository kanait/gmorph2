...
{
  void free_hgppdqtree( HGfc * );
  void FreeHGppdVertex( HGfc * );
  void FreeHGppdEdge( HGfc * );
  void FreeHGppdSurface( HGfc * );
  void FreeHGppdLoop( HGfc * );
  void FreeHGppdCtrlvertex( HGfc * );
...
{
  HGhe *he;
  HGhe *free_hgppdhalfedge( HGhe *, HGsf * );
...
void FreeHGppd( HPpd *ppd )
{
  ...
  free_hgppd( ppd->hgppd1 );
  ppd->hgppd1 = (HGppd *) NULL;
  free_hgppd( ppd->hgppd2 );
...
void HGfcTriangulation( HGfc *hgfc )
{
  int i, n;
  HGsf *sf, *nsf;
  void HGsfTriangulation( HGsf *, HGfc * );
...
/*     display("\tvt %d ed %d lf %d rf %d\n", */
/* 	    he->vt->no, he->ed->no, */
/* 	    ( he->ed->rf != NULL ) ? he->ed->rf->no : SMDNULL, */
/* 	    ( he->ed->lf != NULL ) ? he->ed->lf->no : SMDNULL */
/* 	    ); */
/*   } */
...
/*     display("newed %d sv %d ev %d\n", */
/* 	    newedarray[i]->no, newedarray[i]->sv->no, newedarray[i]->ev->no ); */
  }
  for ( i = 0; i < n - 2; ++i ) {

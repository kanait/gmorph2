...
{
  void sg_adjust_to_parent( int, SGPQHeap * );
...
void sg_adjust_to_parent( int id, SGPQHeap *pqh )
{
  int  p, c;
  int  parent_num( int );
  void swap_sgpqcont( SGPQCont *, SGPQCont * );
...
{
  int  i, a;
  void sg_adjust_to_child( int, SGPQHeap * );
...
/*   if ( a == 58 ) { */
/*     if ( pqh->pqcont[0].tgt_vt != NULL ) { */
/*       display("deleted->next %d\n", pqh->pqcont[0].tgt_vt->no); */
/*     } else { */
/*       display("deleted->next nothing.\n"); */
/*     } */
/*   } */

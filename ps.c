...
/*     fprintf(fp, "%%%%BoundingBox: 0.0 50.0 500.0 450.0\n"); */
/*     fprintf(fp, "%%%%BoundingBox: 30.0 50.0 470.0 450.0\n");  */
...
/* set line-width */
void
pslinewidth(FILE * fp, double width)
{
...
/* eps offset-width */
void
psoffsetwidth(FILE * fp, double d)
{
...
/* print line */
void
psline(FILE * fp, Vec2d * p1, Vec2d * p2)
{
    Vec2d c1, c2;
...
void
psline_4values(FILE * fp, double x1, double y1, double x2, double y2)
{
    fprintf(fp, "%f %f %f %f l\n", x1, y1, x2, y2);
}
...
/* print dashline */
void
psdashline(FILE * fp, Vec2d * p1, Vec2d * p2)
{
    Vec2d c1, c2;
    void  pstran(Vec2d *, Vec2d *);
...
/* print dashline 2 */
void
psdashline2(FILE * fp, Vec2d * p1, Vec2d * p2)
{
    Vec2d c1, c2;
    void  pstran(Vec2d *, Vec2d *);
...
void psrectangle( FILE * fp, Vec2d * p, double len )
{
    ...
    Vec2d c1, c2;
/*     double len; */
/*     void  pscolor(FILE *, double, double, double); */

/*     pscolor(fp, rgb[0], rgb[1], rgb[2]); */
    c1.x = p->x - len;
    c2.x = p->x - len;
    c1.y = p->y - len;
    c2.y = p->y + len;
...
void
psfillrectangle(FILE * fp, Vec2d * p)
{
    Vec2d c;
    void  pstran(Vec2d *, Vec2d *);
...
void
psstar_d(FILE * fp, Vec2d * p, double d)
{
    Vec2d c;
    Vec2d c1, c2;
    void  pstran(Vec2d *, Vec2d *);
...
void
psprintzip(FILE * fp, Vec2d * p, char *text)
{
    Vec2d c;
    void  pstran(Vec2d *, Vec2d *);
...
void
psline_p(FILE * fp, Vec2d * p1, Vec2d * p2)
{
    Vec2d c1, c2;
    void  pstran_p(Vec2d *, Vec2d *);
...
void
psdashline_p(FILE * fp, Vec2d * p1, Vec2d * p2)
{
    Vec2d c1, c2;
    void  pstran_p(Vec2d *, Vec2d *);
...
void
psdashline2_p(FILE * fp, Vec2d * p1, Vec2d * p2)
{
    Vec2d c1, c2;
    void  pstran_p(Vec2d *, Vec2d *);
...
void
psstar_d_p(FILE * fp, Vec2d * p, double d)
{
    Vec2d c;
    Vec2d c1, c2;
    void  pstran_p(Vec2d *, Vec2d *);
...
void
pstext_p(FILE * fp, Vec2d * p, char *text)
{
    Vec2d c;
    void  pstran_p(Vec2d *, Vec2d *);
...
void mhgfc_harmonicmap_to_ps( HPpd *hppd )
{
  ...
  for ( hfc = hppd->shfc; hfc != (HFace *) NULL; hfc = hfc->nxt ) {
    hgfc = hfc->mhgfc;
    sprintf( str, "ps/gppdhrm%d.ps", hfc->no );
    hgfc_harmonicmap_to_ps( str, hgfc );
  }
}
...
void hgfc_harmonicmap_to_ps( char *file, HGfc *hgfc )
{
  ...
  FILE *open_ps( char * );
  void close_ps( FILE * );
  void psline( FILE *, Vec2d *, Vec2d * );
  void psrectangle( FILE *, Vec2d *, double );
  void  pscolor(FILE *, double, double, double);
  void pstext(FILE *, Vec2d *, char *);
  void pslinewidth( FILE *, double );
...
/*     if ( (hgvt->no == 100) || (hgvt->no == 102) || (hgvt->no == 5) || */
/* 	 (hgvt->no == 6) ) { */
/*     if ( hgvt->no == 57 ) { */
/*       sprintf( vname, "V%d", hgvt->no ); */
...
/*     if ( hged->sp_type == SP_EDGE_NORMAL ) { */
/*       pslinewidth( fp, 0.01 ); */
/*     } else { */
/*       pslinewidth( fp, 1.0 ); */
/*     } */
/*     if ( (hged->no == 0) || (hged->no == 22) || (hged->no == 14) ) { */
...
/*       pstext( fp, &v, (char *) vname ); */
/*     } */
/*     if ( (hged->rf == NULL) || (hged->lf == NULL) ) { */
/*       if ( hged->sp_type != SP_EDGE_BOUNDARY ) { */
/* 	display("Warning: hged %d\n", hged->no); */
/*       } */
      psline(fp, &(uvw1), &(uvw2)); 
/*     } */
/*     if ( (hged->rf == NULL) || (hged->lf == NULL) ) { */
/*       sprintf( vname, "E%d", hged->no ); */
/*       v.x = (uvw1.x + uvw2.x) / 2.0; */
/*       v.y = (uvw1.y + uvw2.y) / 2.0; */
/*       pstext( fp, &v, (char *) vname ); */
/*     } */
...
/*     v.x = WINDOWCENTERX + RADIUS * hgvt->uvw.x; */
/*     v.y = WINDOWCENTERY + RADIUS * hgvt->uvw.y; */
/*     if ( (hgvt->no == 6) || (hgvt->no == 8 ) ){ */
/*     sprintf( vname, "v2_%d", hgvt->no ); */
/*     pstext( fp, &v, (char *) vname ); */
/*     } */
/*   } */
      
/*   pscolor( fp, 0.0, 0.0, 255.0 ); */
  for ( hged = hgfc1->shged; hged != (HGed *) NULL; hged = hged->nxt ) {
    ...
    pslinewidth( fp, 0.01 );
    psline( fp, &(uvw1), &(uvw2) );
/*     if ( (hged->no == 8) || (hged->no == 7) || (hged->no == 12) || (hged->no == 13) || */
/* 	 (hged->no == 14) || (hged->no == 15) ) { */
/*     sprintf( vname, "e1_%d", hged->no ); */
/*       v.x = (uvw1.x + uvw2.x) / 2.0; */
/*       v.y = (uvw1.y + uvw2.y) / 2.0; */
/*       pstext( fp, &v, (char *) vname ); */
/*     } */
...
/*     if ( (hged->no == 15) || (hged->no == 17) ) { */
/*     if ( (hged->no == 11) || (hged->no == 25) ) { */
    if ( hged->no == 76 ) {
      sprintf( vname, "e2_%d", hged->no );
      v.x = (uvw1.x + uvw2.x) / 2.0;
      v.y = (uvw1.y + uvw2.y) / 2.0;
...
void harmonicmap_to_ps(char *file, Sppd * ppd)
{
  FILE *fp;
  FILE *open_ps( char * );
  void psharmonicppdedge(FILE *, Sppd *);
  void close_ps(FILE *);

...
static void expandrow(unsigned char *optr, unsigned char *iptr, long z);
static void setalpha(unsigned char *lptr, long n);
static void copybw(long *lptr, long n);
static void lumrow(unsigned char *rgbptr, unsigned char *lumptr, long n); 
static int compressrow(unsigned char *lbuf, unsigned char *rlebuf, long z, long cnt);
static void interleaverow(unsigned char *lptr, unsigned char *cptr, long z, long n);
 ...
 */
#ifdef WIN32
static void bzero(void *ptr, long size)
{
...
unsigned long *
getLongImage(char *textureFile, int *xsize, int *ysize)
{
  void sizeofimage(char *, long *, long *);
  
  sizeofimage(textureFile, (long *) xsize, (long *) ysize);
  return longimagedata(textureFile);
}

void addlongimgtag(unsigned long *dptr, long xsize, long ysize)
{
...
static int writetab(FILE *outf, unsigned long *tab, long len)
{
    int r;
...
static void readtab(FILE *inf, unsigned long *tab, long len)
{
    while(len) {
	*tab++ = getlong(inf);
	len -= 4;
 ...
 *
 */
void sizeofimage(char *name, long *xsize, long *ysize)
{
    IMAGE image;
    FILE *inf;
 ...
 *
 */
int longstoimage(unsigned long *lptr, long xsize, long ysize, long zsize, char *name)
{
    FILE *outf;
    IMAGE *image;
...
char **argv;
{
   ...
   sizeofimage(argv[1],&xsize,&ysize);
    ...
    lbuf = longimagedata(argv[1]);
     longstoimage(lbuf,xsize,ysize,3,"c:\\tmp\\t.rgb");
